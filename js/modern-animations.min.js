        ...
        const stageObserver = new IntersectionObserver(function(entries) {
            entries.forEach(entry => {
                if (entry.isIntersecting && !entry.target.classList.contains('animated')) {
                    ...
                    // Additional guard to prevent double animation
                    if (entry.target.hasAttribute('data-animated')) {
                        console.log(`🎬 [EVOLUTION] Stage ${index + 1} already has data-animated attribute, skipping`);
                        stageObserver.unobserve(entry.target);
                        return;
                    }

                    console.log(`🎬 [EVOLUTION] First time triggering animation for stage ${index + 1}`);
...
function animateStageEntrance(stage, index) {
    ...
    if (stage.hasAttribute('data-animated')) {
        console.log(`🎬 [EVOLUTION] Stage ${index + 1} already has data-animated, skipping`);
        return;
    }

    if (stage.hasAttribute('data-animation-state')) {
        console.log(`🎬 [EVOLUTION] Stage ${index + 1} already has animation state, skipping`);
        return;
    }
...
function initEvolutionScrollTracking() {
    ...
    function updateNavigationOnScroll() {
        ...
        // Only update navigation if the active stage has changed
        if (navState.inJourneySection && navState.activeStageIndex >= 0 && navState.activeStageIndex !== lastActiveStage) {
            updateEvolutionNavigation(navState.activeStageIndex);
            lastActiveStage = navState.activeStageIndex;
            
            // Debug logging only when stage changes
            console.log(`🧭 [EVOLUTION NAV] Stage changed to: ${navState.activeStageIndex + 1}, Progress: ${(navState.progress * 100).toFixed(1)}%`);
        }
    }
...
function initInteractiveEvolution() {
    ...
    stages.forEach((stage, index) => {
        stage.addEventListener('click', function(e) {
            // Prevent navigation update on stage expansion
            e.stopPropagation();
            
            // Only handle expansion if animation is completed
            if (stage.getAttribute('data-animation-state') !== 'completed') {
                console.log(`🎬 [EVOLUTION] Stage ${index + 1} animation not completed, ignoring click`);
                return;
            }
...
function initMouseTrailParticles() {
    ...
    let isMobileOptimized = isMobile;
    ...
    function animate() {
        ...
        if (!isMobileOptimized || Math.random() < 0.5) { // Update half the time on mobile
        ...
        // Skip some background particles on mobile for performance
        const bgStep = isMobileOptimized ? 2 : 1;
        for (let i = 0; i < backgroundParticles.length; i += bgStep) {
...
function initGradientTextAnimation() {
    ...
    function createNameEntranceAnimation() {
        ...
        console.log('🎬 [ENTRANCE] ✅ Entrance animation guards active');
...
// Proper initialization with preloader handling
document.addEventListener('DOMContentLoaded', function() {
    ...
    const preloader = document.getElementById('preloader');
    if (preloader) {
        console.log('🎯 [EVENT DEBUG] Preloader found, waiting for completion...');
        // Initialize preloader and wait for completion
        initPreloader();
    } else {
        console.log('🎯 [EVENT DEBUG] No preloader found, initializing animations directly');
        // Initialize animations immediately if no preloader
        setTimeout(() => {
            console.log('🎯 [EVENT DEBUG] ===== DIRECT ANIMATION INITIALIZATION =====');
            AnimationManager.initialize('no-preloader');
        }, 100);
    }
});
